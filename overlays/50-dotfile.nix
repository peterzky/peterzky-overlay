...

{
  ...
  gfw-update = super.writeShellScriptBin "gfw-update" ''
    update_list() {
        cat /tmp/chnroute.ipset > $HOME/nix/config/files/chnroute.ipset
        echo "List has been updated."
    }

    echo "Downloading China IPs"
    ${super.curl}/bin/curl 'http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest' | grep ipv4 | \
      grep CN | awk -F\| '{ printf("%s/%d\n", $4, 32-log($5)/log(2)) }' | \
    ${super.iprange}/bin/iprange --optimize > /tmp/chnroute.txt
    ...
    diff /tmp/chnroute.ipset $HOME/nix/config/files/chnroute.ipset
