...
def get_workspace(name):
    ...
    want_workspace_cands = [wk for wk in i3.get_workspaces() if wk.name == name]
...
def change_workspace(num):
    ...
    LOG.debug(
        ...
        pformat(focused_workspace, indent=2),
    )

    # Check if already on workspace
    if focused_workspace.name == num:
    ...
    other_output = [
        outp for outp in get_active_outputs() if outp.name == want_workspace.output
    ][0]
    LOG.debug("Other_output=%s", pformat(other_output, indent=2))
    other_workspace = [
        wk for wk in i3.get_workspaces() if wk.name == other_output.current_workspace
    ][0]
    ...
    if focused_workspace.output == want_workspace.output:
        LOG.debug("Wanted workspace already on focused output, " "switching as normal")
        switch_workspace(num)
        return
...
def main():
    ...
    parser.add_argument(
        "--log-file", "-l", metavar="LOG_FILE", help="Path to file to which to log"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Log to stderr")
    args = parser.parse_args()

    setup_logger(logging.DEBUG, log_file=args.log_file, log_stderr=args.verbose)
