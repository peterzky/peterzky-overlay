...
def sensor_proxy_signal_handler(source, changedProperties,
                                invalidatedProperties, **kwargs):
    if source == 'net.hadess.SensorProxy':
        if 'AccelerometerOrientation' in changedProperties:
            ...
            log.info("dbus signal indicates orientation change to %s",
                     orientation)
            subprocess.call(
                ["xrandr", "-o", xrandr_orientation_map[orientation]])
            for device in wacom:
                cmd_and_log([
                    "xsetwacom", "--set", device, "rotate",
                    wacom_orientation_map[orientation]
                ])


# toggle trackpoint and touchpad when changing from laptop to tablet
# mode anc vice versa
def monitor_acpi_events(touch_and_track):
    ...
    while True:
        ...
        eventTabletOn = b'video/tabletmode TBLT 0000008A 00000001\n'
        eventTabletOff = b'video/tabletmode TBLT 0000008A 00000000\n'
        if event == eventTabletOff:
            log.info("display position change detected, tablet mode on")
        ...
        if event == eventTabletOn:
            log.info("display position change detected, tablet mode off")
            for x in touch_and_track:
...
def monitor_stylus_proximity(stylus, finger_touch):
    ...
    for line in out.stdout:
        if (line.startswith(b'proximity')):
            ...
            cmd_and_log([
                "xinput", "disable"
                if status == b'in' else "enable", finger_touch
            ])
...
def main(options):
    ...
    # load wacom devices
    lines = subprocess.check_output(['xsetwacom', '--list',
                                     'devices']).split(b'\n')
    ...
    lines = subprocess.check_output(['xinput', '--list',
                                     '--name-only']).decode().split('\n')
    ...
    # Otherwise enabling/disabling doesn't work as expected and touchpad
    # just stays enabled always
