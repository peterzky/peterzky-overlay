...
(defun wiki-random-entry ()
  ...
  (let* ((wiki-entries
	  (org-map-entries
	   ...
	   '(-filter
	     (lambda (f)
	       (not (string= "/home/peterzky/Sync/sync/org/wiki/index.org" f)))
	     (directory-files-recursively wiki-location "\.org$"))))
	 (marker (nth (random (length wiki-entries)) wiki-entries))
