...
(defvar speed-type--completed-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "l") 'speed-type--log-wpm)
    map))
...
(defvar speed-type--last-wpm 0)
(make-variable-buffer-local 'speed-type-last-wpm)
...
Computes net words-per-minute as:
  ...
  (let ((net-wpm (round (- (speed-type--gross-wpm entries seconds)
                           (/ uncorrected-errors
                              (speed-type--seconds-to-minutes seconds))))))
    (setq speed-type--last-wpm net-wpm)
...
(defun speed-type--handle-complete ()
  ...
  (insert (format "    [%s]og and mark complete\n"
                  (propertize "l" 'face 'highlight)))
...
(defun speed-type--bookmark-text ()
  (let ((begain-marker)
	(end-marker)
	(text))
    (with-temp-buffer
      (bookmark-jump "speed")
      (setq begain-marker (point-marker))
      (forward-word 200)
      (setq end-marker (point-marker))
      (bookmark-set "speed")
      ;; (bookmark-save)
      (setq text (buffer-substring-no-properties
	(marker-position begain-marker)
	(marker-position end-marker)))
      (kill-buffer (current-buffer)))
    text))


(defun speed-type--log-wpm ()
  (interactive)
  (let ((file "/home/peterzky/Sync/sync/org/agenda/area.org")
	(wpm speed-type--last-wpm))
    (with-temp-buffer
      (pop-to-buffer (or (find-buffer-visiting file)
			 (find-file-noselect file)))
      (goto-char (point-min))
      (when (re-search-forward "\*\*\ Typi" nil t)
	(org-todo "DONE"))
      (when (re-search-forward "#\\+name: \\(wpm-table\\)" nil t)
	(goto-char (org-table-end))
	(let ((line (org-table-current-line)))
	  (org-table-goto-line line)
	  (org-table-insert-row t)
	  (org-table-put (1+ line) 1 (format-time-string "[%F %a]") t)
	  (org-table-put (1+ line) 2 (format "%d" wpm) t))))
    (bookmark-save)
...
(defun speed-type-text ()
  (interactive)
  (speed-type--setup (speed-type--bookmark-text)))
